...
{
	m_windowName = L"DX11 Tessellation Grid";
...
bool cViewer::OnInit()
{
	...
	vector<sVertexTex> vertices;
	vertices.reserve(100*100);
	for (int i = 0; i < 100; ++i)
		for (int k = 0; k < 100; ++k)
			vertices.push_back(sVertexTex((float)i*10.f, 0.f, (float)k*10.f, 10.f, 10.f));
...
void cViewer::OnRender(const float deltaSeconds)
{
	...
	if (m_renderer.ClearScene())
	{
		...
		m_renderer.GetDevContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_1_CONTROL_POINT_PATCHLIST);
