...
bool cViewer::OnInit()
{
	...
	GetMainLight().Init(cLight::LIGHT_DIRECTIONAL, 
		Vector4(0.1f, 0.1f, 0.1f, 1), Vector4(0.9f, 0.9f, 0.9f, 1), 
	...
	m_cube.InitCube(m_renderer);
	m_cube.m_mtrl.InitGreen();
	
	m_cube2.InitCube(m_renderer);
	m_cube2.m_tm.SetTranslate(Vector3(4, -2, 2));
...
bool cViewer::GenerateShadowMesh()
{
	...
	hr = D3DXCreateMesh(12, 36, D3DXMESH_32BIT, sShadowVertex::Decl, m_renderer.GetDevice(), &pInputMesh);
	if (FAILED(hr))
		return false;
	...
	{
		pvtx->Position = psrcVtx->p;
		pvtx->Normal = psrcVtx->n;
		++pvtx;
		++psrcVtx;
	}

	for (int i = 0; i < m_cube.m_idxBuff.GetFaceCount()*3; ++i)
		*pidx++ = *psrcIdx++;

	m_cube.m_vtxBuff.Unlock();
	m_cube.m_idxBuff.Unlock();
