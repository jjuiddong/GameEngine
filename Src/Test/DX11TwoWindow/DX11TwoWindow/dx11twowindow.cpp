...
bool cViewer::OnInit()
{
	...
	// Two Window
	m_hWnd2 = framework::InitWindow(m_hInstance
		, L"Window2"
		, sRecti(0, 0, 1024, 768)
		, SW_NORMAL
		, WndProc2
...
void cViewer::OnRender(const float deltaSeconds)
{
	...
	m_renderer.GetDevContext()->OMSetRenderTargets(1, &m_renderTargetView, m_depthStencilView);
	float ClearColor[4] = { 50.f / 255.f, 50.f / 255.f, 50.f / 255.f, 1.0f }; // red,green,blue,alpha
	m_renderer.GetDevContext()->ClearRenderTargetView(m_renderTargetView, ClearColor);
	m_renderer.GetDevContext()->ClearDepthStencilView(m_depthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0);
	{
		if (m_renderer.ClearScene())
		{
			m_renderer.BeginScene();
