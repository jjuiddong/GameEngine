    ...
    ID3D11Texture2D* pCubeTexture, ID3D11ShaderResourceView* pCubeRV )
{
    ...
    // Create the shaders
    //wstring filePath = GetFilePath::GetFilePath(_T("skybox.hlsl"));
	wstring filePath = L"../media/ocean/skybox.hlsl";
    ...
    V_RETURN(graphic::CompileShaderFromFile( (WCHAR *)filePath.c_str(), "SkyboxVS", "vs_4_0", &pBlobVS ) );
    V_RETURN(graphic::CompileShaderFromFile( (WCHAR *)filePath.c_str(), "SkyboxPS", "ps_4_0", &pBlobPS ) );
...
void CSkybox11::OnD3D11ResizedSwapChain( const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    pVertex[0].pos = Vector4( fLowW, fLowH, 1.0f, 1.0f );
    pVertex[1].pos = Vector4( fLowW, fHighH, 1.0f, 1.0f );
    pVertex[2].pos = Vector4( fHighW, fLowH, 1.0f, 1.0f );
    pVertex[3].pos = Vector4( fHighW, fHighH, 1.0f, 1.0f );
...
void CSkybox11::D3D11Render( Matrix44* pmWorldViewProj, ID3D11DeviceContext* pd3dImmediateContext )
{
    //HRESULT hr;
	...
	pVSPerObject->m_WorldViewProj = pmWorldViewProj->Inverse();
