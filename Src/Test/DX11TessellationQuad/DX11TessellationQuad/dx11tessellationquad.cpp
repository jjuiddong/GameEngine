...
{
	m_windowName = L"DX11 Tessellation Quad";
...
bool cViewer::OnInit()
{
	...
	for (int i = 0; i < 100; ++i)
	{
		for (int k = 0; k < 100; ++k)
		{
			...
			//vertices.push_back(sVertexTex((float)i*size, 0.f, (float)k*size + size, size, size));
			//vertices.push_back(sVertexTex((float)i*size + size, 0.f, (float)k*size + size, size, size));
			//vertices.push_back(sVertexTex((float)i*size, 0.f, (float)k*size, size, size));
			//vertices.push_back(sVertexTex((float)i*size + size, 0.f, (float)k*size, size, size));
		}
	}

	m_vtxBuffer.Create(m_renderer, vertices.size(), sizeof(sVertexType), &vertices[0]);

	if (!m_shader.Create(m_renderer, "../media/shader11/tessellation_quad.fxo", "Tech"
